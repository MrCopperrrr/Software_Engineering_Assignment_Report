\part*{1. Triển khai hệ thống}
\addcontentsline{toc}{part}{1. Triển khai hệ thống}

%========================================================================================
\section*{1.1. Sơ đồ triển khai (Deployment View)}
\addcontentsline{toc}{section}{1.1. Sơ đồ triển khai (Deployment View)}
\subsection*{1.1.1 Giới thiệu}
\addcontentsline{toc}{subsection}{1.1.1 Giới thiệu}
...
\subsection*{1.1.2. Sơ đồ triển khai hệ thống}
\addcontentsline{toc}{subsection}{1.1.2. Sơ đồ triển khai hệ thống}
...
\subsection*{1.1.3. Mô tả các thành phần}
\addcontentsline{toc}{subsection}{1.1.3. Mô tả các thành phần}
...
\subsection*{1.1.4. Luồng hoạt động}
\addcontentsline{toc}{subsection}{1.1.4. Luồng hoạt động}
...
\end{enumerate}
%========================================================================================
\section*{1.2. Sơ đồ phát triển (Development View)}
\addcontentsline{toc}{section}{1.2. Sơ đồ phát triển (Development View)}
\subsection*{1.2.1 Giới thiệu}
\addcontentsline{toc}{subsection}{1.2.1 Giới thiệu}
...
\subsection*{1.2.2. Sơ đồ gói (Package Diagram)}
\addcontentsline{toc}{subsection}{1.2.2. Sơ đồ gói (Package Diagram)}
...
\subsection*{1.2.3. Mô tả các gói}
\addcontentsline{toc}{subsection}{1.2.3. Mô tả các gói}
...
\subsection*{1.2.4. Luồng xử lý dữ liệu qua các lớp}
\addcontentsline{toc}{subsection}{1.2.4. Luồng xử lý dữ liệu qua các lớp}
...
\end{itemize}
%========================================================================================
\section*{1.3. Sơ đồ lớp chi tiết (Detailed Class Diagram)}
\addcontentsline{toc}{section}{1.3. Sơ đồ lớp chi tiết (Detailed Class Diagram)}
\subsection*{1.3.1. Giới thiệu}
\addcontentsline{toc}{subsection}{1.3.1. Giới thiệu}
...
\subsection*{1.3.2. Sơ đồ lớp chi tiết của hệ thống}
\addcontentsline{toc}{subsection}{1.3.2. Sơ đồ lớp chi tiết của hệ thống}
...
\subsection*{1.3.3. Mô tả cụm chức năng}
\addcontentsline{toc}{subsection}{1.3.3. Mô tả cụm chức năng}
\begin{itemize}
...
%========================================================================================
\section*{1.4. Mô tả các lớp chi tiết và phương thức}
\addcontentsline{toc}{section}{1.4. Mô tả các lớp chi tiết và phương thức}
\subsection*{1.4.1. Cụm Quản lý Người dùng (User Management)}
\addcontentsline{toc}{subsection}{1.4.1. Cụm Quản lý Người dùng (User Management)}
...
\subsection*{1.4.2. Cụm Chương trình và Đăng ký (Program \& Registration)}
\addcontentsline{toc}{subsection}{1.4.2. Cụm Chương trình và Đăng ký (Program \& Registration)}
...
\subsection*{1.4.3. Cụm Lịch trình và Buổi học (Scheduling \& Session)}
\addcontentsline{toc}{subsection}{1.4.3. Cụm Lịch trình và Buổi học (Scheduling \& Session)}

Cụm này chịu trách nhiệm quản lý thời gian của Tutor, quá trình đặt lịch của sinh viên, và thông tin chi tiết của từng buổi học. Đây là trung tâm của các hoạt động tương tác trong hệ thống.

\textbf{Mô tả:} Đại diện cho một khung thời gian cụ thể mà một \texttt{Tutor} đã đăng ký là rảnh và sẵn sàng để nhận lịch dạy. Đây là ``nguyên liệu'' đầu vào để sinh viên có thể đặt lịch.
...
\textbf{Tên thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\
\hline
- slotId & UUID & Khóa chính, mã định danh duy nhất cho mỗi khung giờ rảnh. \\
- startTime & LocalDateTime & Thời điểm bắt đầu của khung giờ rảnh. \\
- endTime & LocalDateTime & Thời điểm kết thúc của khung giờ rảnh. \\
- mode & SessionMode & Hình thức học trong khung giờ này: ONLINE hoặc OFFLINE. \\
- location & String & Địa điểm (nếu là OFFLINE) hoặc có thể để trống. \\
\hline
\end{tabularx}
\end{table}
...
\textbf{Tên phương thức} & \textbf{Tham số} & \textbf{Kiểu trả về} & \textbf{Mô tả} \\
\hline
+ isSlotAvailable() & (none) & boolean & Kiểm tra xem khung giờ này còn chỗ trống để sinh viên đặt lịch hay không (dựa trên số lượng Session đã được tạo). \\
+ updateSlot() & slotDetails: Map & void & Cho phép Tutor chỉnh sửa thông tin của khung giờ rảnh (ví dụ: thay đổi thời gian, hình thức). \\
\hline
\end{tabularx}
\end{table}

\textbf{Mô tả:} Đại diện cho một buổi học thực tế đã được lên lịch, kết nối một \texttt{Tutor} với một hoặc nhiều \texttt{Student} tại một thời điểm cụ thể. Đây là kết quả của việc sinh viên đặt lịch thành công vào một \texttt{AvailabilitySlot}.
...
\textbf{Tên thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\
\hline
- sessionId & UUID & Khóa chính, mã định danh duy nhất cho mỗi buổi học. \\
- startTime & LocalDateTime & Thời điểm bắt đầu thực tế của buổi học. \\
- endTime & LocalDateTime & Thời điểm kết thúc thực tế của buổi học. \\
- status & SessionStatus & Trạng thái của buổi học (SCHEDULED, COMPLETED, CANCELLED, IN\_PROGRESS). \\
- mode & SessionMode & Hình thức của buổi học (ONLINE hoặc OFFLINE). \\
- locationOrLink & String & Chứa địa điểm phòng học (nếu là OFFLINE) hoặc đường link phòng học trực tuyến (nếu là ONLINE). \\
\hline
\end{tabularx}
\end{table}
...
\textbf{Tên phương thức} & \textbf{Tham số} & \textbf{Kiểu trả về} & \textbf{Mô tả (Liên kết với Use Case)} \\
\hline
+ cancelSession() & reason: String & boolean & Hủy một buổi học đã được lên lịch. Phương thức này sẽ cập nhật status thành CANCELLED (UC-10). \\
+ markAsCompleted() & (none) & void & Đánh dấu buổi học đã hoàn thành. Phương thức này cập nhật status thành COMPLETED (UC-14). \\
+ getAttendees() & (none) & List<Student> & Lấy danh sách tất cả sinh viên đã đăng ký tham gia buổi học này. \\
\hline
\end{tabularx}
\end{table}

\textbf{Mô tả:} Ghi lại thông tin điểm danh của một \texttt{Student} cụ thể cho một \texttt{Session} cụ thể. Lớp này giúp theo dõi sự chuyên cần của sinh viên.
...
\textbf{Tên thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\
\hline
- attendanceId & UUID & Khóa chính, mã định danh duy nhất cho mỗi lượt điểm danh. \\
- status & String & Trạng thái điểm danh: ``PRESENT'' (có mặt), ``ABSENT'' (vắng mặt). \\
- checkInTime & LocalDateTime & Thời điểm Tutor thực hiện điểm danh. \\
\hline
\end{tabularx}
\end{table}
...
\textbf{Tên phương thức} & \textbf{Tham số} & \textbf{Kiểu trả về} & \textbf{Mô tả (Liên kết với Use Case)} \\
\hline
+ updateStatus() & newStatus: String & void & Cập nhật lại trạng thái điểm danh, ví dụ trong trường hợp cần điều chỉnh sau khi điểm danh ban đầu. \\
\hline
\end{tabularx}
\end{table}

\subsection*{1.4.4. Cụm Đánh giá và Phản hồi (Feedback \& Support)}
\addcontentsline{toc}{subsection}{1.4.4. Cụm Đánh giá và Phản hồi (Feedback \& Support)}

Cụm này bao gồm các lớp hỗ trợ cho quá trình dạy và học, cụ thể là quản lý tài liệu học tập và thu thập các phản hồi, đánh giá từ người dùng để cải thiện chất lượng chương trình.

\textbf{Mô tả:} Đại diện cho một phản hồi hoặc đánh giá mà một người dùng gửi cho một người dùng khác, liên quan đến một buổi học cụ thể. Lớp này được thiết kế linh hoạt để hỗ trợ cả hai chiều: sinh viên đánh giá Tutor và Tutor nhận xét về sinh viên.
...
\textbf{Tên thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\
\hline
- feedbackId & UUID & Khóa chính, mã định danh duy nhất cho mỗi phản hồi. \\
- rating & int & Điểm số đánh giá (ví dụ: từ 1 đến 5 sao). \\
- comment & String & Nội dung nhận xét, bình luận chi tiết. \\
- createdAt & LocalDateTime & Dấu thời gian ghi lại thời điểm phản hồi được tạo. \\
\hline
\end{tabularx}
\end{table}

\textbf{Mối quan hệ đặc biệt:} \\
Lớp này có hai mối quan hệ tới lớp \texttt{User}: \texttt{fromUser} (để xác định người gửi) và \texttt{toUser} (để xác định người nhận). \\
Nó cũng có một mối quan hệ tới lớp \texttt{Session} để đảm bảo mỗi phản hồi đều gắn liền với một bối cảnh (buổi học) cụ thể.
...
\textbf{Tên phương thức} & \textbf{Tham số} & \textbf{Kiểu trả về} & \textbf{Mô tả (Liên kết với Use Case)} \\
\hline
+ saveFeedback() & (none) & boolean & Lưu thông tin phản hồi vào cơ sở dữ liệu. Được gọi khi sinh viên hoặc Tutor gửi đánh giá (UC-17, UC-18). \\
+ isValid() & (none) & boolean & Kiểm tra tính hợp lệ của phản hồi (ví dụ: điểm rating phải nằm trong khoảng cho phép). \\
\hline
\end{tabularx}
\end{table}

\textbf{Mô tả:} Đại diện cho một tài liệu học tập (ví dụ: slide bài giảng, bài tập, đề cương) mà \texttt{Tutor} tải lên hệ thống để chia sẻ với các sinh viên trong chương trình học của mình.
...
\textbf{Tên thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\
\hline
- materialId & UUID & Khóa chính, mã định danh duy nhất cho mỗi tài liệu. \\
- fileName & String & Tên gốc của tệp tài liệu. \\
- filePath & String & Đường dẫn lưu trữ tệp trên máy chủ hoặc dịch vụ lưu trữ đám mây. \\
- uploadedAt & LocalDateTime & Dấu thời gian ghi lại thời điểm tài liệu được tải lên. \\
\hline
\end{tabularx}
\end{table}
...
\textbf{Tên phương thức} & \textbf{Tham số} & \textbf{Kiểu trả về} & \textbf{Mô tả (Liên kết với Use Case)} \\
\hline
+ download() & (none) & File & Thực hiện chức năng cho phép người dùng (sinh viên) tải tài liệu về máy (UC-16). \\
+ delete() & (none) & void & Xóa tệp tài liệu khỏi hệ thống. Chỉ người tải lên (Tutor) mới có quyền này (UC-15). \\
+ updateInfo() & newName: String, newDesc: String & void & Cập nhật thông tin mô tả của tài liệu (UC-15). \\
\hline
\end{tabularx}
\end{table}