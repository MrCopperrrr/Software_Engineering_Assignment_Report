% \newpage
\subsubsection*{Use Case 08: Thiết lập lịch dạy (Tutor)}
\begin{samepage}
\begin{table}[h!]
...
\textbf{ID} &
  \multicolumn{3}{l|}{UC-08} &
   \\ \cline{1-4}
\textbf{Tên} &
  \multicolumn{3}{l|}{Thiết lập lịch dạy} &
   \\ \cline{1-4}
\textbf{Mô tả} &
  \multicolumn{3}{l|}{Tutor khai báo thời gian dạy để sinh viên có thể đặt lịch học.} &
   \\ \cline{1-4}
\textbf{Actor chính} &
...
\textbf{Tiền điều kiện} &
  \multicolumn{3}{l|}{Tutor đã đăng ký tài khoản và hồ sơ hợp lệ.} &
   \\ \cline{1-4}
\textbf{Hậu điều kiện} &
  \multicolumn{3}{l|}{Lịch dạy được lưu trong DB và hiển thị cho SV lựa chọn.} &
   \\ \cline{1-4}
\multirow{6}{*}{\textbf{Luồng sự kiện}} &
 ...
 &
  ...
  Đăng nhập, chọn chức năng “Tạo lịch dạy”. &
   \\ \cline{2-4}
 &
  \multicolumn{1}{l|}{2} &
  \multicolumn{1}{l|}{Tutor} &
 ...
 &
  \multicolumn{1}{l|}{5} &
  \multicolumn{1}{l|}{Hệ thống} &
  Hiển thị thông báo “Tạo lịch dạy thành công”. &
   \\ \cline{1-4}
\multirow{3}{*}{\textbf{Luồng thay thế}} &
 ...
 &
  \multicolumn{1}{l|}{3a} &
  \multicolumn{1}{l|}{Hệ thống} &
  \begin{tabular}[c]{@{}l@{}}Phát hiện lịch dạy đã tồn tại → hiển thị lỗi, \\ không lưu.\end{tabular} &
   \\ \cline{1-4}
\textbf{Ngoại lệ} &
  \multicolumn{3}{l|}{Hệ thống lỗi DB khi lưu → hiển thị “Tạo lịch thất bại, thử lại sau”.} &
   \\ \cline{1-4}
\textbf{Business Rules} &
  \multicolumn{3}{l|}{\begin{tabular}[c]{@{}l@{}}Tutor chỉ có thể thiết lập lịch dạy trong khung giờ hợp lệ của trường \\ (ví dụ 7h – 21h).\end{tabular}} &
   \\ \cline{1-4}
\textbf{Data requirement} &
  \multicolumn{3}{l|}{Availability(availID, tutorID, date, startTime, endTime, mode, status)} &
   \\ \cline{1-4}
\end{tabular}
\caption{Bảng đặc tả chức năng thiết lập lịch dạy (Tutor)}
\end{table}
\end{samepage}
...
\textbf{Actor phụ} &
  \multicolumn{3}{l|}{Hệ thống} &
   \\ \cline{1-4}
\textbf{Tiền điều kiện} &
  \multicolumn{3}{l|}{\begin{tabular}[c]{@{}l@{}}SV đã đăng ký chương trình học và đã được ghép Tutor cho môn học \\ (UC-06 hoặc UC-07). \\ Tutor đã khai báo lịch dạy (UC-08).\end{tabular}} &
   \\ \cline{1-4}
\textbf{Hậu điều kiện} &
...
\multirow{9}{*}{\textbf{Luồng sự kiện}} &
 ...
 &
  \multicolumn{1}{l|}{4} &
  \multicolumn{1}{l|}{Hệ thống} &
  Hiển thị danh sách slot dạy của Tutor. &
   \\ \cline{2-4}
 &
  \multicolumn{1}{l|}{5} &
  \multicolumn{1}{l|}{Sinh viên} &
...
\multirow{4}{*}{\textbf{Luồng thay thế}} &
 ...
 &
  \multicolumn{1}{l|}{4a} &
  \multicolumn{1}{l|}{Hệ thống} &
  \begin{tabular}[c]{@{}l@{}}Không có slot dạy phù hợp \\ → báo “Chưa có lịch khả dụng”.\end{tabular} &
   \\ \cline{2-4}
 &
  \multicolumn{1}{l|}{6a} &
  \multicolumn{1}{l|}{Hệ thống} &
...
\multirow{6}{*}{\textbf{Luồng sự kiện}} &
 ...
 &
  \multicolumn{1}{l|}{5} &
  \multicolumn{1}{l|}{Sinh viên} &
  \begin{tabular}[c]{@{}l@{}}Nếu đổi:\\ Hệ thống hiển thị danh sách dạy khác của Tutor.\\ SV chọn 1 slot mới.\\ Hệ thống kiểm tra:\\     Slot chưa full.\\     Không trùng với lịch học khác của SV.\\ Nếu hợp lệ → Hệ thống cập nhật slot mới cho \\ toàn bộ môn học.\\ Hệ thống gửi thông báo cho cả SV và Tutor.\end{tabular} &
   \\ \cline{1-4}
\multirow{3}{*}{\textbf{Luồng thay thế}} &
  \multicolumn{1}{l|}{\textbf{Bước}} &
  \multicolumn{1}{l|}{\textbf{Thực hiện bởi}} &
